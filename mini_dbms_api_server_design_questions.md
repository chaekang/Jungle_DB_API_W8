...
- 서버 재시작 후 데이터가 살아있어야 하는가?
- 메모리 기반으로만 갈 것인가, 파일 저장까지 할 것인가?
- 매 요청마다 flush할 것인가, 종료 시 저장할 것인가?
- 인덱스는 같이 저장할 것인가, 서버 시작 시 rebuild할 것인가?
- 하루짜리 구현 기준에서 가장 현실적인 저장 전략은 무엇인가?
...
- 이 프로젝트의 한 줄 차별점은 무엇인가?
- 왜 이 프로젝트가 단순한 toy server가 아니라 설계된 시스템처럼 보이는가?
- 어떤 지표를 보여줄 수 있는가?
  - QPS
  - latency
  - 동시 요청 수
  - index 사용 시 / 미사용 시 차이
- 가장 적은 비용으로 인상적인 추가 기능은 무엇인가?
  - 응답에 `execution_time_ms` 포함
  - 응답에 `index_used: true/false` 포함
  - `/metrics` endpoint
  - slow query logging
  - request logging
  - benchmark script
