...
int main(void) {
    ...
    tokens = tokenizer_tokenize_with_error(
    ...
    if (assert_true(tokens != NULL, "tokenizer_tokenize should return tokens") != SUCCESS ||
        assert_true(token_count == 19, "tokenizer should tokenize full INSERT") != SUCCESS ||
    ...
    tokens = tokenizer_tokenize_with_error("Select * FROM users WHERE age >= 27;",
                                           &token_count, error, sizeof(error));
    if (assert_true(tokens != NULL, "tokenizer should support SELECT") != SUCCESS ||
    ...
    tokens = tokenizer_tokenize_with_error("DELETE FROM users WHERE name = 'Alice';",
                                           &token_count, error, sizeof(error));
    if (assert_true(tokens != NULL, "tokenizer should support DELETE") != SUCCESS ||
    ...
    tokens = tokenizer_tokenize_with_error("SELECT 'oops", &token_count,
                                           error, sizeof(error));
    if (assert_true(tokens == NULL, "unterminated string should fail") != SUCCESS ||
        assert_true(strstr(error, "Unterminated string literal") != NULL,
                    "unterminated string should expose a structured error") != SUCCESS) {
        free(tokens);
        return EXIT_FAILURE;
    }

    if (assert_true(tokenizer_get_cache_entry_count() == 0,
                    "tokenizer cache should stay disabled") != SUCCESS ||
        assert_true(tokenizer_get_cache_hit_count() == 0,
                    "tokenizer cache hit count should stay zero") != SUCCESS) {
        return EXIT_FAILURE;
    }
