...
int main(void) {
    ...
    if (assert_true(tokenizer_get_cache_entry_count() == 0,
                    "tokenizer should not keep shared cache entries") != SUCCESS ||
        assert_true(tokenizer_get_cache_hit_count() == 0,
                    "cache hit count should remain zero without cache") != SUCCESS) {
        tokenizer_cleanup_cache();
        return EXIT_FAILURE;
    }
    ...
    if (assert_true(tokens != NULL, "repeated parse should still return tokens") != SUCCESS ||
        assert_true(tokenizer_get_cache_entry_count() == 0,
                    "cache size should stay zero on repeated SQL") != SUCCESS ||
        assert_true(tokenizer_get_cache_hit_count() == 0,
                    "repeated SQL should not rely on cache hits") != SUCCESS) {
        free(tokens);
        tokenizer_cleanup_cache();
        return EXIT_FAILURE;
    }
